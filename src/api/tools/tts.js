...
async function tts(text) {
  ...
  const requests = Object.entries(models).map(async ([modelName, { voice_id, voice_name }]) => {
    ...
    try {
      ...
      // PERBAIKAN: Cek apakah data ada dan struktur sesuai
      if (!tobrut_milik_ins || !tobrut_milik_ins.data || !tobrut_milik_ins.data.convert_result || 
          !Array.isArray(tobrut_milik_ins.data.convert_result) || tobrut_milik_ins.data.convert_result.length === 0) {
        console.error(`Response tidak valid untuk model ${modelName}:`, tobrut_milik_ins);
        return {
          modelName,
          error: `Response tidak valid dari API untuk model ${modelName}`
        };
      }
      ...
      console.error(`Yah, ada yang salah nih pas nyobain untuk model ${modelName}:`, error.message);
      return {
        modelName,
        error: `Waduh, kayaknya ada yang gak beres nih untuk model ${modelName}: ${error.message}`
      };
    }
  });
  ...
  const data = ceker_babat_punya_ins.map(({ modelName, channel_id, oss_url, voice_id, voice_name, error }) => {
    ...
    // PERBAIKAN: Cek apakah data yang diperlukan ada
    if (!channel_id || !oss_url) {
      return {
        modelName,
        error: `Data tidak lengkap untuk model ${modelName}`
      };
    }
...
module.exports = function(app) {
  app.get('/tools/text-to-speech', async (req, res) => {
    ...
    try {
      ...
      console.error("Error pada endpoint TTS:", error);
      res.status(500).json({
        status: false,
        error: error.message
      });
